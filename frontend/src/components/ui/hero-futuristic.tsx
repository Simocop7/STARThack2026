...
const PostProcessing = ({
  ...
  useFrame(({ clock }) => {
    ...
    if (typeof anyRender.render === "function") {
      anyRender.render();
    } else if (typeof anyRender.renderAsync === "function") {
      anyRender.renderAsync();
    }
  }, 1);
...
const Scene = () => {
  ...
  const { material, uniforms } = useMemo(() => {
    ...
    // Use a single float channel from the depth texture for type-safety in three/tsl.
...
export const Html = () => {
  ...
  useEffect(() => {
    // Only client: generate random delays for glitch
